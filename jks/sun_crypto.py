...
def jce_pbe_encrypt(data, password):
    ...
    salt = os.urandom(8)
    iteration_count = CRandom.randint(1500, 4000)
...
def _jce_pbe_derive_key_and_iv(password, salt, iteration_count):
    if len(salt) != 8:
        raise BadDataLengthException("Expected 8-byte salt for PBEWithMD5AndTripleDES (OID %s), found %d bytes" % (".".join(str(i) for i in SUN_JCE_ALGO_ID), len(salt)))
