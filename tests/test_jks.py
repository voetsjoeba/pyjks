...
class BksLoadTests(AbstractTest):
    ...
    def test_bks_bad_sealedkey_decrypt(self):
        # Here's a keystore such that when you try to decrypt the SealedKeyEntry called "sealed_public_key" with the password "store_password",
        # the result is a bad decrypt that we should be able to detect (i.e. decrypts to garbage plaintext while still passing the PKCS#5 padding check).
        ...
        # equivalently:
        entry = jks.BksSealedKeyEntry(alias="alias", timestamp=int(time.time())*1000, cert_chain=[], encrypted=entry_data)
